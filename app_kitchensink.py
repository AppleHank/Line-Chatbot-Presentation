...
def handle_text_message(event):
    ...
    if text == 'profile':
        ...
        url = request.url_root + '/static/cheems.jpg'
        print(url)
...
def handle_content_message(event):
    ...
    #send picture to server to calculate similarity with database
    ...
    print('get response')
    #retrieve calculate result, send to user
    data=resp.json()
    ...
    #情緒分析
    # try:
    #     emotion = facial_emotion_model.emotion_recognition(img,print_prob=True)
    #     message = f'分析情緒：{emotion}'
    # except AttributeError:
    #     print('no face captured')
    #     message = '無法捕捉臉部，請嘗試上傳更高解析度 / 確認臉部垂直於地面'
    # line_bot_api.reply_message(
    #     event.reply_token, [
    #         TextSendMessage(text=message),
    #         TextSendMessage(text=f"圖片網址：{request.host_url + path}"),
    #         TextSendMessage(text=request.host_url + 'test.jpg')
    #     ])
